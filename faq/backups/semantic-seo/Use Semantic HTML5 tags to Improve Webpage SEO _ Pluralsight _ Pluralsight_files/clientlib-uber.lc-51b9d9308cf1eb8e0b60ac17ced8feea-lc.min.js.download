...
(function () {
  ...
  const init = () => {
    ...
    /* Marketo forms have been configured to always include a X18_digit_SF_Lead_ID__c field, but it will start empty.
    We fetch the X18_digit_SF_Lead_ID__c value from Marketo.
    Only users that have previously filled out one of our forms will have this SalesForce ID.
    We will use this as a unified ID between Marketo, Salesforce, and Adobe Analytics. */
    const X18_FIELD_NAME = 'X18_digit_SF_Lead_ID__c';
    const handleMarketoX18Prefill = () => {
      const x18element = jQuery(`input[name="${X18_FIELD_NAME}"]`);
      const x18container = x18element.closest('.mktoFormRow');
      x18container.hide(); // this field cannot be type="hidden" but it must not be visible to the end user

      // Gate clause -- skip prefill if they've opted out of cookies/tracking
      if (Utils.cookies.getCookie('ps_optout') === 1) {
        return;
      }

      // Gate clause -- skip ajax request if x18 value is already been fetched
      const x18Cookie = Utils.cookies.getCookie('ps_mkto-x18');
      if (x18Cookie) {
        prefillX18(x18Cookie);
        return;
      }

      // Gate clause -- skip ajax request if _mkto_trk is missing
      const marketoCookie = Utils.cookies.getCookie('_mkto_trk');
      if (!marketoCookie) {
        return;
      }

      const marketoCookieUrlEncoded = encodeURIComponent(marketoCookie);
      const marketoCookieBase64Encoded = btoa(marketoCookieUrlEncoded);
      let x18Value = null;
      jQuery.ajax({
        url: `/bin/marketo-prefill?mkto-trk=${marketoCookieBase64Encoded}`,
        method: "GET",
        async: true,
        headers: {
          'Content-Type': 'application/json; charset=utf-8'
        ...
        error: function () {
          console.warn(`Marketo API errored for _mkto_trk of ${marketoCookie}`);
        },
        success: function (data) {
          if (data && data?.success === true) {
            ...
            console.log(`Marketo API not reporting success=true for _mkto_trk of ${marketoCookie}`);
          }
        },
        complete: function() {
          if (x18Value) {
            // store this x18 value client-side to prevent overusing the marketo API
            Utils.cookies.setCookie('ps_mkto-x18', x18Value, null, null, 30);
          }
        }
      });
    };

    const prefillX18 = (x18Value) => {
      const $x18element = jQuery(`input[name="${X18_FIELD_NAME}"]`);
      $x18element.val(x18Value);
    };
